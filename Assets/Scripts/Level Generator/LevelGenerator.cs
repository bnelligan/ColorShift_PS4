...
public class LevelGenerator : MonoBehaviour {
    ...
    public MapNode CreateNode(MapNode parentNode, Gate parentGate)
    {
        ...
        newGate = newNode.FindClosestGate(parentGate, 1000);
    ...
    public MapNode CreateNode_NEW(MapNode parentNode, Gate parentGate)
    {
        // Initialize return vars
        MapNode newNode = null;
        Gate newGate = null;
        ...
        // Set initial new position to the parent position
        Vector3 newPos = parentNode.transform.position;
        GameObject goNode = Instantiate(nodePrefab.gameObject, newPos, parentNode.transform.rotation);
        Debug.Log("Created node: " + goNode.name);
        newNode = goNode.GetComponent<MapNode>();

        /*
        // Move the new node by a half it's size
        if (parentGate.direction == Direction.RIGHT)
            newNode.WorldPosition += new Vector2(newNode.Width + parentNode.Width + 1, 0)/2;
        else if (parentGate.direction == Direction.LEFT)
            newNode.WorldPosition -= new Vector2(newNode.Width + parentNode.Width + 1, 0)/2;
        else if (parentGate.direction == Direction.UP)
            newNode.WorldPosition += new Vector2(0, newNode.Height + parentNode.Height + 1)/2;
        else if(parentGate.direction == Direction.DOWN)
            newNode.WorldPosition -= new Vector2(0, newNode.Height + parentNode.Height + 1)/2;*/
        newNode.Init();

        // Connect the gates with the path
        newGate = newNode.FindClosestGate(parentGate, 100);

        // Move the nodes so each gate overlaps
        Vector2 sepVec = parentGate.WorldPosition - newGate.WorldPosition;
        newNode.WorldPosition += sepVec;
        newNode.ValidatePosition();
